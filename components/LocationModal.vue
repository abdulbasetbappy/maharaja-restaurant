<template>
  <vue-final-modal v-slot="{ params, close }" v-bind="$attrs" classes="modal-container" content-class="modal-content"
    :drag="false" id="main-modal">
    <div class="i-order-type">
      <div class="i-button-group">
        <button class="i-button-delivery" :class="{ active: !cart.is_pickup }" @click="cart._is_pickup = false">
          <font-awesome-icon icon="truck" />
          {{ $t("Delivery") }}
        </button>
        <button class="i-button-pickup" :class="{ active: cart.is_pickup }" @click="cart._is_pickup = true">
      ...
      <div class="i-select-location" v-if="cart.is_pickup">
        <select class="i-select" name="location" id="location" ref="location"
          @change="goToLocation($event.target?.value)">
          <option value="0" disabled selected>
            {{ $t("Select location") }}
          </option>
          <option v-for="(location, key) in settings?.settings?.locations" :key="key" :value="location.id">
            {{ location.address }}, {{ location.city }} - {{ location.zip }},
            {{ location.state }}
          </option>
        </select>

        <svg class="i-select-icon" fill="currentColor" height="16px" width="16px" xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 330 330" xml:space="preserve" stroke-width="2">
          <path
      ...
      <div class="i-select-location" v-else>
        <select class="i-select" name="location" id="location" ref="location"
          ...
          <option v-for="(location, key) in settings?.settings?.delivery_zones" :key="key" :value="location.id">
